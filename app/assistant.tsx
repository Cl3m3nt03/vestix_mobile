...
export default function Assistant() {
  ...
  const [pending, setPending] = useState('')   // bulle assistant en cours d'écriture
    ...
    <AppShell>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={8}>
        ...
        {me.isLoading ? (
          ...
          <Animated.View entering={FadeInUp.duration(280)} style={styles.pad}>
            <FxCard>
              ...
              <Text style={styles.consentTxt}>
                L'assistant transmet un résumé de ton patrimoine à Google Gemini pour personnaliser
                ses réponses. Aucune donnée n'est partagée sans ton accord (RGPD).
              </Text>
              ...
              {streaming ? (
                <Animated.View entering={FadeIn.duration(160)}>
                  <View style={[styles.bubble, styles.bot]}>
                    {pending ? (
                      <Text style={styles.bubbleTxt}>{pending}</Text>
                    ) : (
                      <TypingDots />
                    )}
                  </View>
...
const styles = StyleSheet.create({
  ...
  // Chat
  chat: { paddingHorizontal: 16, paddingVertical: 12, gap: 10, paddingBottom: 18 },
  emptyWrap: { alignItems: 'center', gap: 10, marginTop: 40, paddingHorizontal: 8 },
  emptyBadge: {
